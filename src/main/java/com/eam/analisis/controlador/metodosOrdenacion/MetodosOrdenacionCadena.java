...
public class MetodosOrdenacionCadena {
    ...
    public static void ordenarMonticulo(ArrayList<Cancion> arreglo) {
        ...
        for (int heapsize = 0; heapsize < arreglo.size(); heapsize++) {
            ...
            while (n > 0) { // until we reach the root of the heap
                ...
                if (arreglo.get(n).getNombre().compareTo(arreglo.get(p).getNombre()) > 0) { // el índice del int insertado
        ...
        for (int heapsize = arreglo.size(); heapsize > 0;) {
            ...
            while (true) {
                ...
                if (right >= heapsize) { // El nodo tiene un hijo izquierdo, pero ningún hijo derecho
                    if (arreglo.get(left).getNombre().compareTo(arreglo.get(n).getNombre()) > 0) // si el hijo izquierdo es mayor que el nodo
                    {
                        cambiarPosicion(arreglo, left, n); // intercambiar hijo izquierdo con nodo
                    }
                    break; // el montón es heapified
                }
                if (arreglo.get(left).getNombre().compareTo(arreglo.get(n).getNombre()) > 0) { // (left > n)
                    if (arreglo.get(left).getNombre().compareTo(arreglo.get(right).getNombre()) > 0) { // (left > right) & (left > n)
                    ...
                    if (arreglo.get(right).getNombre().compareTo(arreglo.get(n).getNombre()) > 0) { // (right > n > left)
                        cambiarPosicion(arreglo, right, n);
                        n = right;// continuar la recursión en el niño derecho
                    } else { // (n > left) & (n > right)
    ...
    public static void ordenarBurbuja(ArrayList<Cancion> arreglo) {
        ...
        do {
            ...
            for (int i = 0; i < arreglo.size() - iteracion; i++) {
                if (arreglo.get(i).getNombre().compareTo(arreglo.get(i + 1).getNombre()) > 0) {
                    permutacion = true;
    ...
    public static void ordenarInsercion(ArrayList<Cancion> array) {
        ...
        for (x = 1; x < array.size(); x++) { // desde el segundo elemento hasta el final
            ...
            while ((y >= 0) && (aux.getNombre().compareTo(array.get(y).getNombre())) < 0) { // mientras queden posiciones y el valor de aux sea menor que los de la izquierda
    ...
    public static void ordenarSeleccion(ArrayList<Cancion> array) {
        ...
        for (i = 0; i < array.size() - 1; i++) { // tomamos como menor el primero
            ...
            for (j = i + 1; j < array.size(); j++) { // buscamos en el array algun elemento menor al actual
                if (array.get(j).getNombre().compareTo(menor.getNombre()) < 0) {
                    menor = array.get(j);
                    poss = j;
                }
            }
    ...
    public static void ordenarPeine(ArrayList<Cancion> array) {
        ...
        while ((permutación) || (gap > 1)) {
            ...
            for (actual = 0; actual < array.size() - 1 - gap; actual++) {
                if (array.get(actual).getNombre().compareTo(array.get(actual + gap).getNombre()) > 0) {
                    permutación = true;
    ...
    public static void ordenarBurbujaBidireccional(ArrayList<Cancion> arreglo) {
        ...
        do {
            ...
            while (((direccion == 1) && (actual < fin)) || ((direccion == -1) && (actual > comienzo))) {
                actual += direccion;
                if (arreglo.get(actual).getNombre().compareTo(arreglo.get(actual - 1).getNombre()) < 0) {
    ...
    private static void burbuja(ArrayList<Cancion> arreglo, int p) {
        int j = p;
        while ((j > 0) && (arreglo.get(j).getNombre().compareTo(arreglo.get(j-1).getNombre()) < 0)) {
        ...
        while (salto > 0) {//Mientras el salto sea diferente de 0 se seguira ordenando
            for (int i = salto; i < arreglo.size(); i++) {//Recorrer el arrglo desde la pocision del salto
                ...
                while (j >= 0) {//Mientras el salto no sea 0, esto significa que tomara los valores desde atras a adelante,
                    ...
                    if (arreglo.get(k).getNombre().compareTo(arreglo.get(j).getNombre()) >= 0) {
    ...
    private static void quickSort(ArrayList<Cancion> arreglo, int posIzquierda, int posDerecha) {
        ...
        while (posIzquierda != posDerecha) {
            while (arreglo.get(posDerecha).getNombre().compareTo(arreglo.get(pivote).getNombre()) >= 0 && posIzquierda < posDerecha) {
                posDerecha--;// cambiara de posicion si todo esta bien
            }            // por el lado derecho
            while (arreglo.get(posIzquierda).getNombre().compareTo(arreglo.get(pivote).getNombre()) < 0 && posIzquierda < posDerecha) {
